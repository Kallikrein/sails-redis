...
Indice.prototype.index = function index(value, cb) {
  ...
  this._indexed(value, function(err, indexed) {
    ...
    if(indexed === 1) return cb(new Error(Errors.adapter.NotUnique));

    // Create the index
    console.log(['KALLIKREIN DEBUG :  index', self.keyName, value].join(':'));
    self.client.set([self.keyName, value].join(':'), true, function(err) {
      if(err) return cb(err);
      cb();
    });
  });
...
Indice.prototype._indexed = function _indexed(value, cb) {
  console.log(['KALLIKREIN DEBUG :  _indexed', this.keyName, value].join(':'));
  this.client.exists([this.keyName, value].join(':'), function(err, indexed) {
    if(err) {console.log('Y A UNE ERREUR ICI : ' + err);return cb(err);}
    cb(null, indexed);
  });
};
