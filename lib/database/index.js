...
Database.prototype.drop = function drop(collectionName, relations, cb) {
  ...
  this.connection.smembers(index, function(err, values) {
    ...
    async.each(keys, function(key, next) {
      console.log('KALLIKREIN DEBUG key in drop method');
      console.log(key);
      self.connection.keys(key + '*', function(err, hits) {
        if (err) return cb(err);
        console.log('KALLIKREIN DEBUG loggin key() return');
        console.log('key : ' + key);
        console.log('hits : ' + hits);
        self.connection.del(hits, next);
      });
      //next();
...
Database.prototype.find = function find(collectionName, criteria, cb) {
  ...
  console.log('KALLIKREIN DEBUG FIND KEYS :');
  ...
  if ( 
    ...
    console.log('THIS HAS BEEN TRIGGERD');
  ...
  async.auto({
    ...
    getSetMembers: function(next) {
      console.log('KALLIKREIN DEBUG : getsetMembers');
      console.log('indexKey : ' + indexKey);
      self.connection.keys(indexKey + '*', function(err, members) {
        if(err) {console.log('y avait une erreur ici ?' + err);return next(err);}
        console.log('hits : ' + members);
        next(null, members);
      });
    },
    ...
    filterKeys: ['cleanseCriteria', 'getSetMembers', function(next, results) {
      ...
      console.log('KALLIKREIN DEBUG filterKeys members : ' + members);

      function search(member, done) {
        console.log('KALLIKREIN DEBUG search : member' + member);
        self.connection.get(member, function(err, result) {
          console.log('KALLIKREIN DEBUG search : result' + result);
...
Database.prototype.create = function create(collectionName, data, cb) {
  ...
  async.auto({
    ...
    // primary key should be used to set
    ...
    addToSet: ['saveKeyData', function(next, results) {
      ...
      // Grab the key to use for this record
      var noSqlKey = self.schema.recordKey(name, primary, data[primary]);
      console.log('KALLIKREIN DEBUG addToSet');
      console.log('noSqlKey : ' + noSqlKey);
      self.connection.set(noSqlKey, true, function(err) {
        if(err) return next(err);
        next();
      });
    ...
    updateIndexes: ['checkUniqueConstraints', 'checkPrimaryKeyValid', function(next, results) {
      ...
      function updateIndex(idx, nextItem) {
        ...
        data.forEach(function(record) {
          multi.del([key, record[name]].join(':'));
        });

        // Add the new indexed value
        multi.set([key, values[name]].join(':'), true);
...
Database.prototype.destroy = function destroy(collectionName, criteria, cb) {
  ...
  async.auto({
    ...
    // Find all matching records based on the criteria
    findRecords: function(next) {
      console.log('KALLIKREIN DEBUG destroy->findRecords Called');
      // Use the FIND method to get all the records that match the criteria
      self.find(name, criteria, function(err, records) {
        if(err) {console.log('KALLIKREIN DEBUG somehow find raised an error');return next(err);}
        console.log('KALLIKREIN DEBUG destroy->findrecords->find');
        console.log(records);
        next(null, records);
      });
    },
    ...
    destroyIndexes: ['findRecords', function(next, results) {
      ...
      // Cache original values
      var data = results.findRecords;
      console.log('KALLIKREIN DEBUG : destroyIndexes : data : ' + data);
      // Get Indices
      var indices = self.schema._indices[name];
      ...
      function destroyIndex(idx, nextItem) {
        ...
        data.forEach(function(record) {
          multi.del([key, record[name]].join(':'));
          console.log(['KALLIKREIN DEBUG : destroyIndexes ', key, record[name]].join(':'));
        });
      ...
      data.forEach(function(item) {
        ...
        console.log('KALLIKREIN DEBUG data.forEach : recordKey : ' + recordKey);
        console.log('KALLIKREIN DEBUG data.forEach : indexKey : ' + indexKey);
        multi.del([indexKey, Utils.sanitize(recordKey)].join(':'));
      });
...
Database.prototype._uniqueConstraint = function _uniqueConstraint(collectionName, data, cb) {
  ...
  indices.forEach(function(idx) {
    ...
    multi.exists([idx.keyName, data[idx.name]].join(':'));
  });
